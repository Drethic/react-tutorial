    ...
    render() {
        ...
        if (this.state.showPersons) {
            persons = ( < Persons persons = {
                ...
                changeName = {
                    this.stateModifyPersonNameHandler
                }
                changeAge = {
                    this.stateModifyPersonAgeHandler
                }
                />
            );
        }
        ...
        if (this.state.showOutput) {
            output = ( < div > {
                    this.state.names.map((name, key) => ( < UserOutput name = {
                            name
                        }
                        key = {
                            key
                        ...
                        change = {
                            this.stateModifyHandler.bind(this, key)
                        }
                        />
                    ))
                } < /div>);
            ...
            if (this.state.showTask1) {
                task1 = ( < div >
                    ...
                    elements in the components - both with inline styles and stylesheets < /li> < /ol > < button className = "btn"
                    onClick = {
                        this.toggleOutput
...
if (this.state.showTask2) {
    task2 = ( < div >
            < ol >
            < li > Create an input field( in App component) with a change listener which outputs the length of the entered text below it(e.g.in a paragraph). < /li> < li > Create a new component( => ValidationComponent) which receives the text length as a prop < /li > < li > Inside the ValidationComponent, either output "Text too short"
            or "Text long enough"
            depending on the text length(e.g.take 5 as a minimum length) < /li> < li > Create another component( => CharComponent) and style it as an inline box( => display: inline - block, padding: 16 px, text - align: center, margin: 16 px, border: 1 px solid black). < /li > < li > Render a list of CharComponents where each CharComponent receives a different letter of the entered text( in the initial input field) as a prop. < /li> < li > When you click a CharComponent, it should be removed from the entered text. < /li > < /ol> < p > Hint: Keep in mind that JavaScript strings are basically arrays! < /p > < input type = "text"
            onChange = {
                this.changeHandler
            }
            value = {
        ...
        } < /p><Length length={this.state.text.length} / >
        < Char text = {
            this.state.text
        }
    delete = {
        this.deleteLetterHandler
    }
    /> < /div >
);
}
...
if (this.state.showMainApp) {
    ...
    mainApp = ( < div >
        ...
        } > This is really working! < /p> < Persons persons = {
        this.state.appPersons
    }
    clicked = {
        this.deletePersonHandler
    }
    changeName = {
        this.stateModifyAppPersonNameHandler
    }
    changeAge = {
        this.stateModifyAppPersonAgeHandler
    }
    /> < /div >
);
}
