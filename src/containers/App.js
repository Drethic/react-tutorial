...
class App extends Component {
  state = {
    ...
    persons: [
      {id: 'firstPerson', name: 'first', age: 23},
      {id: 'secondPerson', name: 'second', age: 27},
      {id: 'thridPerson', name: 'thrid', age: 42},
      {id: 'fourthPerson', name: 'bob', age: 12},
    ],
    appPersons: [
      {id: 'firstPerson', name: 'first', age: 23},
      {id: 'secondPerson', name: 'second', age: 27},
      {id: 'thridPerson', name: 'thrid', age: 42, hobbies: 'Racing'},
      {id: 'fourthPerson', name: 'bob', age: 12},
    ],
    text: '',
    showOutput: false,
  ...
  toggleTask1 = (event) => {
    this.setState({showTask1: !this.state.showTask1});
  };
  ...
  stateModifyAppPersonNameHandler = (key, event) => {
    ...
    const personIndex = persons.findIndex(p => (p.id === key));
    persons[personIndex].name = event.target.value;
    this.setState({appPersons: persons});
  }
  ...
  render() {
      ...
      output = (
        <div>
          {this.state.names.map((name, key) => (
            <UserOutput name={name} key={key} change={this.stateModifyHandler.bind(this, key)} />
          ))}
        </div>
      );
    }
    ...
    if (this.state.showAge) {
      age = (
        <div><p>States for Ages array: {this.state.ages.join(', ')}</p></div>
      );
    }
